...
class ArgoverseV1DataModule(pl.LightningDataModule):
    ...
    def __init__(self,
        ...
        #self.sample_val_rate = 100

    def prepare_data(self) -> None:
        ArgoverseV1Dataset(self.root, 'train', self.train_transform, self.num_historical_steps, self.num_future_steps, self.margin, self.sample_rate)
        ArgoverseV1Dataset(self.root, 'val', self.val_transform, self.num_historical_steps, self.num_future_steps, self.margin, 1)

    def setup(self, stage: Optional[str] = None) -> None:
        self.train_dataset = ArgoverseV1Dataset(self.root, 'train', self.train_transform, self.num_historical_steps, self.num_future_steps, self.margin, self.sample_rate)
        self.val_dataset = ArgoverseV1Dataset(self.root, 'val', self.val_transform, self.num_historical_steps, self.num_future_steps, self.margin, 1)
