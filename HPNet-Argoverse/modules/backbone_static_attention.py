...
from layers import GraphAttentionReturenW
...
class Backbone_Static(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_Static, self).__init__()
        ...
        self.l2m_attn_layer = GraphAttentionReturenW(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
        self.t2m_attn_layer = GraphAttentionReturenW(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
        ...
        self.l2n_attn_layer = GraphAttentionReturenW(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
        self.t2n_attn_layer = GraphAttentionReturenW(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        m_embs_t,t2m_weight = self.t2m_attn_layer(x=[t_embs, m_embs], edge_index=t2m_edge_index,
                                       edge_attr=t2m_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]

        # l2m attention
        m_embs_l,l2m_weight = self.l2m_attn_layer(x=[l_embs, m_embs], edge_index=l2m_edge_index,
        ...
        # t2n attention
        t_embs = a_embs.reshape(-1, self.hidden_dim)  # [(N1,...,Nb)*H,D]
        n_embs_t,t2n_weight = self.t2n_attn_layer(x=[t_embs, n_embs], edge_index=t2n_edge_index,
                                       edge_attr=t2n_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]

        # l2m attention
        n_embs_l,l2n_weight = self.l2n_attn_layer(x=[l_embs, n_embs], edge_index=l2n_edge_index,
        ...
        return traj_propose, traj_output, prob_output, \
            t2m_weight, l2m_weight, t2n_weight, l2n_weight  # [(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K]