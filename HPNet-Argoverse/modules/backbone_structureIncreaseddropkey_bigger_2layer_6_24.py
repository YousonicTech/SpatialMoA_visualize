...
from layers import SpaceMaskGraphAttention
from layers import KeyDropGraphAttention
#from layers import StructureKeyDropGraphAttention
from layers import StructureIncreasedKeyDropGraphAttention
...
class Backbone_StructureIncreasedDropKey_Bigger2layer_6_24(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_StructureIncreasedDropKey_Bigger2layer_6_24, self).__init__()
        ...
        self.l2m_attn_layer = StructureIncreasedKeyDropGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                                      has_edge_attr=True, if_self_attention=False, mask_ratio=0.05,num_r_bins = 6,num_theta_bins = 24)
        self.t2m_attn_layer = StructureIncreasedKeyDropGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                                      has_edge_attr=True, if_self_attention=False, mask_ratio=0.1,num_r_bins = 6,num_theta_bins = 24)
        ...
        self.l2n_attn_layer = StructureIncreasedKeyDropGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                                      has_edge_attr=True, if_self_attention=False, mask_ratio=0.025,num_r_bins = 6,num_theta_bins = 24)   #0.05 #0.1
        self.t2n_attn_layer = StructureIncreasedKeyDropGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                                      has_edge_attr=True, if_self_attention=False, mask_ratio=0.05,num_r_bins = 6,num_theta_bins = 24)  #0.1  #0.2
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        far_enough = t2m_edge_attr_length > 35
        parallel = t2m_edge_attr_heading.abs() < (15 / 180) * 3.1415926
        to_mask = far_enough & parallel  # 48
        ...
        far_enough_l = l2m_edge_attr_length > 40
        parallel_l = l2m_edge_attr_heading.abs() < (30 / 180) * 3.1415926
        to_mask_l = far_enough_l & parallel_l
        ...
        m2m_a_valid_mask = m2m_a_valid_mask.unsqueeze(2) & m2m_a_valid_mask.unsqueeze(
            1)  # [H*K,(N1,...,Nb),(N1,...,Nb)]
        m2m_a_valid_mask = drop_edge_between_samples(m2m_a_valid_mask, m2m_a_batch).contiguous()
        ...
        n2n_a_valid_mask = n2n_a_valid_mask.unsqueeze(2) & n2n_a_valid_mask.unsqueeze(
            1)  # [H*K,(N1,...,Nb),(N1,...,Nb)]
        n2n_a_valid_mask = drop_edge_between_samples(n2n_a_valid_mask, n2n_a_batch).contiguous()
        ...
        n_embs_t = self.t2n_attn_layer(x=[t_embs, n_embs], edge_index=t2n_edge_index,
                                       edge_attr=t2n_edge_attr_embs,raw_attr=t2n_edge_attr_input)  # [(N1,...,Nb)*H*K,D]

        # l2m attention
        n_embs_l = self.l2n_attn_layer(x=[l_embs, n_embs], edge_index=l2n_edge_index,
                                       edge_attr=l2n_edge_attr_embs,raw_attr = l2n_edge_attr_input)  # [(N1,...,Nb)*H*K,D]
...
if __name__ == "__main__":
    ...
    num_heads = 8
    dropout = 0.1
    BackboneOnlyRegister_instance = Backbone_StructureIncreasedDropKey_Bigger1layer_6_24(
    ...
    data = torch.load('/Users/queenie/Documents/HPNet_MoARegister/HPNet-Argoverse/data_1109_20251109_113054_437.pt',
                      map_location=torch.device('cpu'))
    l_embs = torch.load('/Users/queenie/Documents/HPNet_MoARegister/HPNet-Argoverse/l_embs_1109_20251109_113054_437.pt',
                        map_location=torch.device('cpu'))
    a, b, c = BackboneOnlyRegister_instance.forward(data, l_embs)
