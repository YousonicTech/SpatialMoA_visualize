...
from layers import GraphAttentionAdaptiveMoSAAttGate
...
class Backbone_MoA_Bigger1layer_6_24(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_MoA_Bigger1layer_6_24, self).__init__()
        ...
        self.l2m_attn_layer = GraphAttentionAdaptiveMoSAAttGate(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
        self.t2m_attn_layer = GraphAttentionAdaptiveMoSAAttGate(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
                                             has_edge_attr=True, if_self_attention=False)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        m_embs_t,_ = self.t2m_attn_layer(x=[t_embs, m_embs], edge_index=t2m_edge_index,
                                       edge_attr=t2m_edge_attr_embs)  # [(N1,...,Nb)*H*K,D]

        # l2m attention
        m_embs_l,_ = self.l2m_attn_layer(x=[l_embs, m_embs], edge_index=l2m_edge_index,
        ...
        return traj_propose, traj_output, prob_output  # [(N
