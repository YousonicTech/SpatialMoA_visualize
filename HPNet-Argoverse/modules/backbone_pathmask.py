...
class Backbone_PathMask(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads: int,
                 dropout: float) -> None:
        super(Backbone_PathMask, self).__init__()
        ...
        self.l2m_attn_layer = SpaceMaskGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, has_edge_attr=True, if_self_attention=False,mask_ratio = 0.005)
        self.t2m_attn_layer = SpaceMaskGraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, has_edge_attr=True, if_self_attention=False,mask_ratio = 0.2)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        m_embs_t = self.t2m_attn_layer(x = [t_embs, m_embs], edge_index = t2m_edge_index, edge_attr = t2m_edge_attr_embs,att_mask = to_mask)         #[(N1,...,Nb)*H*K,D]

        #l2m attention
        m_embs_l = self.l2m_attn_layer(x = [l_embs, m_embs], edge_index = l2m_edge_index, edge_attr = l2m_edge_attr_embs, att_mask = to_mask_l)         #[(N1,...,Nb)*H*K,D]
...
if __name__ == "__main__":
    ...
    data = torch.load('/Users/queenie/Documents/HPNet_MoARegister/HPNet-Argoverse/data_1109_20251109_113054_437.pt',map_location=torch.device('cpu'))
    l_embs = torch.load('/Users/queenie/Documents/HPNet_MoARegister/HPNet-Argoverse/l_embs_1109_20251109_113054_437.pt',map_location=torch.device('cpu'))
