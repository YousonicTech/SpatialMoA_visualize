...
class SpaceMaskGraphAttention(MessagePassing):
    ...
    def __init__(self,
                 ...
                 mask_ratio:float,
                 **kwargs) -> None:
        super(SpaceMaskGraphAttention, self).__init__(aggr='add', node_dim=0, **kwargs)
    ...
    def forward(self,
        ...
        x_dst = x_dst + self._mha_layer(x_src, x_dst, edge_index, edge_attr,att_mask)
    ...
    def message(self,
        ...
        # att_mask_exp = att_mask.unsqueeze(-1).expand(-1, self.num_heads)
        # true_idx = att_mask_exp.nonzero(as_tuple=False)
        # K = true_idx.size(0) # [K, 2], 每行是 [e_idx, head_idx]
        if sum(att_mask) > 0 and self.training:
            edge_idx = att_mask.nonzero(as_tuple=False).squeeze(-1)
            N = edge_idx.size(0)
            if N > 0:
                num_drop_edge = max(1, int(self.mask_ratio * N))  # 比如 10% * 48 ≈ 4~5 条 edge
                perm = torch.randperm(N, device=weight.device)
                drop_edge_idx = edge_idx[perm[:num_drop_edge]]  # [num_drop_edge]

                # 对这些被选中的 edge，在所有 head 上都置为 -inf
                weight[drop_edge_idx, :] = float('-inf')
    ...
    def _mha_layer(self,
                   ...
                   edge_attr: Optional[torch.Tensor]=None,
                   att_mask=None) -> torch.Tensor:
        return self.propagate(edge_index=edge_index, edge_attr=edge_attr, x_dst=x_dst, x_src=x_src, att_mask=att_mask)
