...
class HPNet(pl.LightningModule):
    ...
    def training_step(self,data,batch_idx):
        traj_propose, traj_output, prob_output,t2m_att, l2m_att, t2n_att, l2n_att  = self(data)               #[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K]
    ...
    def validation_step(self,data,batch_idx):
        traj_propose, traj_output, prob_output,t2m_att, l2m_att, t2n_att, l2n_att  = self(data)
        if batch_idx == 1000:
            return
        att_dict ={
            "t2m_att":t2m_att,
        ...
        torch.save(att_dict, str(batch_idx) + ".pt")#[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K,F,2],[(N1,...,Nb),H,K]
        target_traj, target_mask = generate_target(position=data['agent']['position'], 
                                                   mask=data['agent']['visible_mask'],
                                                   num_historical_steps=self.num_historical_steps,
                                                   num_future_steps=self.num_future_steps)  #[(N1,...Nb),H,F,2],[(N1,...Nb),H,F]
